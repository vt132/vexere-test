            ...
            if url.endswith('/intents/plan'):
                ...
                if mode == 'full_change_time':
                    return MockResp(json_data={
                        ...
                        "action": {"name": "update_ticket_time", "args": {"order_id": 12, "new_time_iso": "2025-09-15T10:00:00"}}
                    })
                if mode == 'trips':
                    return MockResp(json_data={
                        "intent": "get_trips",
                ...
                if mode == 'faq':
                    return MockResp(json_data={
                        ...
                        "action": {"name": "faq", "args": {"question": json.get("text") if json else "FAQ?"}}
                    })
            if url.endswith('/agent/change_time'):
                return MockResp(json_data={"answer": "Updated", "tool_calls": [], "tool_results": []})
            if url.endswith('/orders/update_time'):
                return MockResp(json_data={"updated": True, "order": {"order_id": 12, "departure_time": "2025-09-15T10:00:00"}})
...
def test_change_time_full_exec(client, plan_mode):
    ...
    resp = client.post('/intents/plan', json={"text": "Đổi giờ vé order 12 sang 2025-09-15T10:00:00", "user_id": 7})
